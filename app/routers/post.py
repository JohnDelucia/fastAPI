...
from ..database import engine, get_db  # .. goes up a directory
...
def get_posts(db: Session = Depends(get_db),
              limit: int = 10, skip: int = 0, search: Optional[str] = ""):  # added query parameters (shows up after ? in url)
...
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db),
    ...
    new_post = models.Post(id=current_user.id, **post.dict())
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(Oauth2.get_current_user)):
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Not authorized to perform requested access")
